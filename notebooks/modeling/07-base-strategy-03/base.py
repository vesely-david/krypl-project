...
class BaseStrategy(Strategy):
    def __init__(self, exchange, data_manager, contract_pair, willing_loss, target_return):
    ...
    def trade(self):
        ...
        while self.data_manager.has_tick():
            ...
            if not self.opened and _return != last_return and _return <= self.target_return:
                ...
                self.target_profit = total_target * self.profit_targets[target_i]
                for i in range(2):
                    try:
                        self.buy(price)
                        break
