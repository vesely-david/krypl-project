...
class ModelingData:
    def __init__(self, data, columns_to_drop, modeling_start, strategy_testing_start):
        def split_trading_data():
            self.strategy_testing_data = self.filter_df(f'date >= "{self.strategy_testing_start}"')
            self.modeling_data = self.filter_df(f'"{self.modeling_start}" <= date < "{self.strategy_testing_start}"')
